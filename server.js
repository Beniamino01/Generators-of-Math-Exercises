...
async function resolveExistingPublicFile(fileNames) {
  ...
  return path.join(publicDir, fileNames[0]);
}
...
app.get("/", async (_, res, next) => {
  try {
    const filePath = await resolveExistingPublicFile([
      "index.html",
      "Sitocompleto_modificato.html",
      "index_old.html"
    ]);
...
app.get("/complete-site", async (_, res, next) => {
  try {
    const filePath = await resolveExistingPublicFile([
      "index.html",
      "Sitocompleto_modificato.html",
